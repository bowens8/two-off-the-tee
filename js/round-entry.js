...
async function loadCourses() {
  try {
    ...
    toast(`Couldn't load courses: ${e.code || e.message}`);
  }
}
...
async function loadRecentRounds() {
  ...
  try {
    ...
    slot.innerHTML = snap.docs.map(d => {
      ...
      const statusPill = r.status === 'completed' ? `<span class="pill sand">Finished</span>` : `<span class="pill flag">In progress</span>`;
      return `<a class="round-list-item" href="round-entry.html?round=${d.id}">
        ...
        <div class="rli-tags">${tags}${statusPill}</div></a>`;
    }).join('');
  } catch (e) {
    console.error('Failed to load recent rounds:', e);
    slot.innerHTML = `<p class="loading-line">Couldn't load rounds: ${esc(e.code || e.message)}. Check your Firestore rules.</p>`;
  }
}
...
document.getElementById('start-round-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!playerNames[state.user.uid]) playerNames[state.user.uid] = state.profile.name;
    ...
    toast(`Couldn't start the round: ${e.code || e.message}. Check your Firestore rules / config.`);
  } finally {
    btn.disabled = false;
...
document.getElementById('finish-round-btn').addEventListener('click', async () => {
  if (!currentRoundRef) return;
  const btn = document.getElementById('finish-round-btn');
  btn.disabled = true;
  try {
    await updateDoc(currentRoundRef, { status: 'completed', completedAt: Date.now() });
    toast('Round marked as finished — nice one.');
    if (unsubscribeRound) { unsubscribeRound(); unsubscribeRound = null; }
    history.replaceState(null, '', 'round-entry.html');
    document.getElementById('scorecard-view').style.display = 'none';
    document.getElementById('round-picker').style.display = '';
    ...
    toast(`Couldn't finish the round: ${e.code || e.message}`);
  } finally {
    btn.disabled = false;
  }
});
...
function openScorecard(roundId) {
  ...
  unsubscribeRound = onSnapshot(currentRoundRef, (snap) => {
    if (!snap.exists()) { toast('This round no longer exists.'); return; }
    const round = { id: snap.id, ...snap.data() };

    // make sure current user is registered as a player on this round —
    // fire this in the background so a blocked write can't strand the UI;
    // render with the player added locally either way.
    if (!round.players.includes(state.user.uid)) {
      round.players = [...round.players, state.user.uid];
      round.playerNames = { ...round.playerNames, [state.user.uid]: state.profile.name };
      updateDoc(currentRoundRef, {
        players: round.players,
        [`playerNames.${state.user.uid}`]: state.profile.name
      }).catch(e => {
        console.error('Failed to join round:', e);
        toast(`Couldn't save that you joined this round: ${e.code || e.message}`);
      });
    }
    ...
    const chText = round.courseHandicaps && round.courseHandicaps[subUidA] != null
      ...
      : '';
    const statusText = round.status === 'completed' ? 'Round finished ✓' : (round.players.length < 2 ? 'Waiting for the other player to join…' : 'Both players connected');
    document.getElementById('sv-sub').textContent = `${round.date}${round.time ? ' · ' + round.time : ''} · ${statusText}${chText}`;
    ...
    toast(`Couldn't load this round: ${err.code || err.message}. Check your Firestore rules.`);
  });
}
