...
async function saveSoloRound() {
  ...
  try {
    ...
    console.error('Failed to save solo round:', e);
    toast(`Couldn't save that round: ${e.code || e.message}. Check your Firestore rules.`);
  } finally {
    btn.disabled = false;
  }
}
...
async function renderExisting() {
  ...
  let snap;
  try {
    snap = await getDocs(query(collection(db, 'soloRounds'), orderBy('date', 'desc')));
  } catch (e) {
    console.error('Failed to load solo rounds:', e);
    historySlot.innerHTML = `<p class="loading-line">Couldn't load solo rounds: ${esc(e.code || e.message)}. Check your Firestore rules.</p>`;
    return;
  }
